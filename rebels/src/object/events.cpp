...
PostSystemInitEvent::PostSystemInitEvent(EventType event_type) : Event(event_type) {}
...
BarEvent::BarEvent(EventType event_type, std::vector<Order> orders)
    : Event(event_type), orders(orders) {}
...
OrderPendingNewEvent::OrderPendingNewEvent(EventType event_type, Order& order)
    : Event(event_type), order(order) {}

OrderCreationPassEvent::OrderCreationPassEvent(EventType event_type, Order& order)
    : Event(event_type), order(order) {}

OrderCreationRejectEvent::OrderCreationRejectEvent(EventType event_type, Order& order)
    : Event(event_type), order(order) {}

OrderPendingCancelEvent::OrderPendingCancelEvent(EventType event_type, Order& order)
    : Event(event_type), order(order) {}

OrderCancelPassEvent::OrderCancelPassEvent(EventType event_type, Order& order)
    : Event(event_type), order(order) {}

OrderCancelRejectEvent::OrderCancelRejectEvent(EventType event_type, Order& order)
    : Event(event_type), order(order) {}

OrderUnsolicitedUpdateEvent::OrderUnsolicitedUpdateEvent(EventType event_type, Order& order)
    : Event(event_type), order(order) {}

TradeEvent::TradeEvent(EventType event_type, Order& order, Trade& trade)
    : Event(event_type), order(order), trade(trade) {}
