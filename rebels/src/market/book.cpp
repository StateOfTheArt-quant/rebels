...
void Book<C, DEPTH>::ApplyChanges(
    ...
    for (unsigned int l = 0; l < DEPTH; l++) {
        if (new_prices[l] <= 0.0)
            throw std::runtime_error("Prices must be non-zero positive: " + std::to_string(new_prices[l]));
        else if (new_volumes[l] <= 0)
            throw std::runtime_error("Volumes must be non-zero positive: " + std::to_string(new_prices[l]));
        else {
...
double Book<C, DEPTH>::price(int level)
{
    ...
    if ((unsigned int) level >= DEPTH or level < 0 or prices[level] == 0.0)
        throw std::runtime_error("Attempted to access an undefined price at level: " +
                            std::to_string(level));
    else
        return prices[level];
}
...
double Book<C, DEPTH>::last_price(int level)
{
    ...
    if ((unsigned int) level >= DEPTH or level < 0 or last_prices[level] == 0.0)
        throw std::runtime_error("Attempted to access undefined last price at level: " +
                            std::to_string(level));
    else
...
bool Book<C, DEPTH>::PlaceOrder(double price, long size,char side)
{
    ...
    if (OrderExists(it))
        ...
        open_orders.emplace(price, std::make_unique<Order>(price, size, volume(price),side));
